...
In a smart grid environement, we study coalitions of prosumers (agents that produce but also consume depending on time) which aim at selling energy to the grid. We reckon that the grid is able to specify two requirements (stability and minimum production) that enable a coalition to enter the market. We then design an algorithmm that seeks for uncorrelated profiles among the prosumers in order to form coalitions that are statistically stable enough to pass the grid requirements. The proposed algorithm uses complex system tools such as correlation graphs or clique percolation, and remains simple enough such that coalitions are formed in reasonable time. We validate the algorithm against a random procedure and show that, it not only performs better in term of social welfare, but that it is also more robust against grid minimum production threshold variations.  
