...
\begin{figure}
\begin{tabular}{cc}
   \includegraphics[scale=0.44]{./figure1/util.eps} &
...
Through this paper, our interest was on how we could circumscribe groups of prosumers that can rise above the grid requirements. In this direction, we removed seasonal variations from the prosumers timeseries and proposed to organize them over a correlation graph with a reversed metric. A greedy clique percolation process on the $ \epsilon $-graph enables us to finally establish coalitions of agents. We showed in the last section that our algorithm was actually finding coalitions with high utility values but also that it was far more robust than a random search over coalition structures when the grid operator increases his requirements (Fig. 4).
