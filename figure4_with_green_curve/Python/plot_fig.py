...
Pmax = 12; phi = 0.15; nb_coal = 7
axes = plt.gca()
axes.set_ylim([0,1.2])
axes.set_xlim([0,11])
plt.rc('text', usetex=True)
plt.errorbar(range(Pmax), perco, yerr = std_perco, fmt='o--', color= 'b', label='percolation')
plt.errorbar(range(Pmax), rand, yerr = std_rand, fmt='o--', color= 'r', label='random')
plt.errorbar(range(Pmax), corre, yerr = std_corre, fmt='o--', color= 'g', label='correlated')
plt.legend(loc='upper center', bbox_to_anchor=(0.45, 1.05), ncol=3, fancybox=True, shadow=True)
plt.xlabel(r" \textbf{ $ P_{\phi}^{MIN}$ }",fontsize=14)
